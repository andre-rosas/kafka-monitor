  ...
  (fn [f]
    ...
    (with-redefs [db/connect! (constantly nil)
                  ...
                  config/producer-config (constantly {:rate-per-second 10
                                                      ...
                                                      :batch-size 16384
                                                      :linger-ms 1})
                  config/orders-config (constantly {:customer-id-range [1 100]
...
;; =============================================================================
;; Configuration-Based Generation Tests
;; =============================================================================

(deftest test-order-generation-respects-config-ranges
  (testing "Generated orders respect all configuration ranges"
    (dotimes [_ 25]
      (let [order (core/generate-order)
            orders-config (config/orders-config)
            [min-cust max-cust] (:customer-id-range orders-config)
            [min-qty max-qty] (:quantity-range orders-config)
            [min-price max-price] (:price-range orders-config)
            product-ids (:product-ids orders-config)]

        ;; Customer ID in range
        (is (>= (:customer-id order) min-cust))
        (is (<= (:customer-id order) max-cust))

        ;; Quantity in range
        (is (>= (:quantity order) min-qty))
        (is (<= (:quantity order) max-qty))

        ;; Price in range
        (is (>= (:unit-price order) min-price))
        (is (<= (:unit-price order) max-price))

        ;; Valid product ID
        (is (contains? (set product-ids) (:product-id order)))))))
